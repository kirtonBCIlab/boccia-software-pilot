...
def read_xdf(file: str, picks: list[str]="all"):
    ...
    for stream_id in range(len(data)):
        ...
        if data[stream_id]["info"]["source_id"][0]=="SMARTING" and data[stream_id]["info"]["type"][0]=="EEG":
            eeg_np = data[stream_id]["time_series"]
            srate = float(data[stream_id]["info"]["nominal_srate"][0])
            break

        # source_id_list = data[stream_id]["info"]["source_id"][0].split("_")
        # if source_id_list[0] == 'gUSBamp' and source_id_list[-1] != "markers":
        #     eeg_np = data[stream_id]["time_series"]
        #     srate = float(data[stream_id]["info"]["nominal_srate"][0])
        #     break

        stream_name = data[stream_id]["info"]["name"][0]
    ...
    n_chans = len(data[stream_id]['info']['desc'][0]['channels'][0]['channel'])
    chans_names = [data[stream_id]['info']['desc'][0]['channels'][0]['channel'][i]['label'][0] for i in range(n_chans)]
    # chans_dict = {
    ...
    return eeg_pd.to_numpy().T, srate
